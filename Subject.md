...
- [I. Preamble](#i-preamble) — 2
- [II. Foreword](#ii-foreword) — 3
- [III. AI Instructions](#iii-ai-instructions) — 4
- [IV. Introduction](#iv-introduction) — 7
- [V. Common Instructions](#v-common-instructions) — 8
  - V.1 General Rules — 8
  - V.2 Makefile — 8
  - V.3 Additional Guidelines — 9
  - V.4 Additional Requirements — 9
- [VI. Mandatory part](#vi-mandatory-part) — 10
...
- [VII. Evaluation](#vii-evaluation) — 19
  - VII.1 Evaluation metrics — 19
    - VII.1.1 Recall@k Calculation — 19
    - VII.1.2 Performances — 20
- [VIII. Readme Requirements](#viii-readme-requirements) — 21
- [IX. Bonus Part](#ix-bonus-part) — 23
- [X. Submission and peer-evaluation](#x-submission-and-peer-evaluation) — 24
...
flowchart LR
    subgraph Indexing
        A["Corpus<br/>(data/raw/vllm-0.10.1)"] --> B["Chunking<br/>(code | markdown)"]
        B --> C["Index<br/>BM25 / TF-IDF<br/>(data/processed)"]
    end

    subgraph "Retrieval and Generation"
        D["Retrieve<br/>top-k sources"] --> E["Augment<br/>(build context)"]
        E --> F["Generate<br/>(Qwen3-0.6B)"]
        F --> G["Answer (JSON)"]
        D -.scoring.-> H["Evaluate<br/>recall@k (IoU > 0.05)"]
    end
...
**Example — indexing a target corpus:**
...
Implement the following `pydantic` models; they validate the data exchanged between the stages.

**`MinimalSource`** — represents a single source of information:
...
**`UnansweredQuestion`** and **`AnsweredQuestion`** — represent an unanswered question and an answered question:
...
**`MinimalSearchResults`** and **`MinimalAnswer`** — represent the search results and an answer:
